...
int main(int argc, char **argv)
{
    ...
    //Sins and initial alignment define
    CVect3 pos0, alignatt;
    pos0.i = lat;
    pos0.j = lon;
    pos0.k = high;
    CAligni0 aln(pos0);
    ...
    foreach (rosbag::MessageInstance const m, view)
    {
        ...
        if (imu_msg)
        {
            ...
            if (cnt<(frq*align0T))   //Initial alignment process for sins using inertial frame i0 method
            {
                pImuGps->vm.i=imu_msg->linear_acceleration.x*TS;
                pImuGps->vm.j=imu_msg->linear_acceleration.y*TS;
                pImuGps->vm.k=imu_msg->linear_acceleration.z*TS;
                pImuGps->wm.i=imu_msg->angular_velocity.x*TS;
                pImuGps->wm.j=imu_msg->angular_velocity.y*TS;
                pImuGps->wm.k=imu_msg->angular_velocity.z*TS;
                aln.Update(&pImuGps->wm, &pImuGps->vm, 1, TS);
                alignatt = q2att(aln.qnb);
            }
            else if (cnt==(frq*align0T))  //Initial alignment finish
            {
                double v0[3]={0, 0 ,0};
                pImuGps->t=imu_msg->header.stamp.toSec();
                sins.Init(aln.qnb,v0,pos0,pImuGps->t);
            }
            else
            {
                //ins solve
                ...
                pImuGps->wm.j=imu_msg->angular_velocity.y*TS;
                pImuGps->wm.k=imu_msg->angular_velocity.z*TS;
                sins.Update(&pImuGps->wm, &pImuGps->vm,1,TS);
                ...
                data.rot = Eigen::Quaterniond(sins.qnb.q0,
                                              sins.qnb.q1,
                                              sins.qnb.q2,
                                              sins.qnb.q3);
